...
int main()
{
    ...
    */
    ...
    auto xvector = XMLoadFloat4(&xfloat4); // 로드를 하고 계산 후
    xvector = XMVector3Length(xvector); // sqrt(1*1 + 2*2 + 3*3), 함수 이름이 XMVector 숫자3 Length() 입니다.

    float length;
    XMStoreFloat(&length, xvector); // length로 받아옴
    // 사용이유 : simd 가속을 이용해서 32bit float를 4개씩 묶어 128bit 를 한번에 계산
    // 가속에 특화되어 사용이 번거로울 수 있음
