...
public:
	Example(HWND window, int width, int height)
	{
		...
		*/
		...
		// 이미지 밝게 : 주의 색의 값이 범위이상 초과할시 색이 깨져버리는 현상이 나타남
		// => clamp로 범위 제한을 해주기
		for (int i = 0; i < image.height * image.width; i++)
		{
			image.pixels[i].v[0] += 1e-2f; // 0.01f
			image.pixels[i].v[1] += 1e-2f;
			image.pixels[i].v[2] += 1e-2f;
	...
	{
		//전체 조금씩 어두워지게
		for (auto& it : image.pixels)
		{
			it.v[0] = std::clamp(it.v[0] * 0.9f, 0.0f, 1.0f);
			it.v[1] = std::clamp(it.v[1] * 0.9f, 0.0f, 1.0f);
			it.v[2] = std::clamp(it.v[2] * 0.9f, 0.0f, 1.0f);
		}
	}

	void LightDark() 
	{
		//화면절반 밝아지게 , 나머지 절반 어두워지게
		for (int i = 0; i < image.width; i++)
		{
			for (int j = 0; j < image.height; j++)
			{
				image.pixels[i + image.width * j].v[0] *= (i < image.width / 2 ? 0.9f : 1.1f);
				image.pixels[i + image.width * j].v[1] *= (i < image.width / 2 ? 0.9f : 1.1f);
				image.pixels[i + image.width * j].v[2] *= (i < image.width / 2 ? 0.9f : 1.1f);
			}
		}
	}
	void Update()
