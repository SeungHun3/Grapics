...
#include <iostream>
#include <d3d11.h> 
#include <d3dcompiler.h> // vertex shader (vs.hlsl) 와 pixel shader (ps.hlsl) 을 컴파일 할때 사용 
#include <vector>
...
public:
	...
	// 윈도우 해상도와 그림을 draw해주는 해상도가 다르기에 각각 넣어줌
	...
	void Update()
	{
		...
		unsigned int y = x / 10;
		// canvasWidth * canvasHeight 의 배열에 rgba 넣고
		std::vector<Vec4> pixels(canvasWidth * canvasHeight, Vec4{ backgroundColor[0], backgroundColor[1], backgroundColor[2], 1.0f});
		pixels[y + canvasWidth * 0]
			= y % 3 == 0 ? Vec4{ 1.0f, 0.0f, 0.0f, 1.0f }
			: y % 3 == 1 ? Vec4{ 0.0f, 1.0f, 0.0f, 1.0f }
			: Vec4{ 0.0f, 0.0f, 1.0f, 1.0f };

		// Update texture buffer
		D3D11_MAPPED_SUBRESOURCE ms;
		// gpu의 메모리와 cpu 메모리를 canvasTexture 라는 메모리 영역으로 대응시킴
		deviceContext->Map(canvasTexture, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &ms);
		// gpu메모리에 cpu 메모리에 있는 pixel데이터를 복사하여
		memcpy(ms.pData, pixels.data(), pixels.size() * sizeof(Vec4));
		// gpu 메모리를 닫아줌
		deviceContext->Unmap(canvasTexture, NULL);
		y < (pixels.size() - 1) ? x++ : x = 0;

		// 메모리 복사과정에 있어 병목현상(bottle neck)이 일어남
		// =>그렇기에 가급적 메모리 복사에 있어 줄이기 위해 노력해야함 
	}
